...
<body>
			...
			<div class="col-lg-12">
				<div class="panel panel-default">
					<div class="panel-heading">用户列表
						<a href="<c:url value='/managers/user/add.jsp' />" class="btn btn-primary" style="position: relative;float: right;margin:10px;">添加</a>
					</div>
					
					<form action="<c:url value='/friendLink/delete.do' />" id="body" name="body" method="post">
						<div class="panel-body">
							<table data-toggle="table" data-url="<c:url value='/user/queryUserOrdinary.do' />"  
							...
							data-sort-name="name" data-sort-order="desc"  data-id-field="id">
							    ...
							    <tr>
							       <th data-field="ids" data-checkbox="true" ></th>
							        ...
							        <th data-field="operate" data-formatter="operateFormatter" data-events="operateEvents">设为管理员</th>
							    </tr>
							    </thead>
							</table>
						</div>
    ...
    function operateFormatter(value, row, index) {
        return [
            '<a class="edit ml10" href="javascript:void(0)" title="设置管理员">',
    ...
    window.operateEvents = {
        'click .edit': function (e, value, row, index) {
        	 if(confirm("确认将当前用户设置为管理员？"))
        	    {
            		var id=row["id"];
            		window.location.href="<c:url value='/user/toEditManager.do?id="+id+"'  />";
        	    }
        }
    };
</script>
